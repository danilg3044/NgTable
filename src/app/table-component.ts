...
import {MatPaginator, MatSort, MatTableDataSource, MatTable, MatDialog,
                  MatSnackBar, MatSnackBarConfig, MatDialogConfig, MatSelect} from '@angular/material';
import {DatePipe} from '@angular/common';
import {ErrorStateMatcher} from '@angular/material/core';
import {FormControl, NgModel, FormGroupDirective, NgForm, FormGroup, FormBuilder} from '@angular/forms';
import { HttpClientService, Employee } from './http-client/http-client.service';
import { webSocket, WebSocketSubject } from 'rxjs/websocket';
import {SavePromptComponent} from './save-prompt/save-prompt';
...
export class TableComponent implements OnInit, OnDestroy {
  displayedColumns: string[] = ['index', 'id-sep', 'id', 'email-sep', 'email', 'salary-sep', 'salary', 'quality-sep',
                                        'quality', 'date-sep', 'date', 'locked-sep', 'locked', 'tools-sep', 'tools'];
  filterColumns: string[] =    ['index-filter', 'id-filter-sep', 'id-filter', 'email-filter-sep', 'email-filter',
          ...
          'date-filter', 'locked-filter-sep', 'locked-filter', 'tools-header-sep', 'tools-header'];
  ...
  @ViewChild('salaryEdit')  salaryEdit: ElementRef;
  @ViewChild('qualityEdit')   qualityEdit: MatSelect;
  @ViewChild('dateEdit')    dateEdit: ElementRef;
  @ViewChild('lockEdit')    lockEdit: ElementRef;
  ...
  constructor(private formBuilder: FormBuilder, private http: HttpClientService, private dialog: MatDialog,
              private snackBar: MatSnackBar, private datePipe: DatePipe, private toasterService: ToasterService) {
  ...
  isEditable(row) {
    return this.editable && this.editable === row.id;
  }

  openSnackBar(message: string, action: string) {
    const config = new MatSnackBarConfig();
    config.panelClass = ['snackbar', 'success'];
    config.duration = 2048;

    this.snackBar.open(message, action, config);
  }

  openDialog(id: number): void {
    const email = this.emailEdit.nativeElement.value;
    const salary = +this.salaryEdit.nativeElement.value;
    const date = this.dateEdit.nativeElement.value;
    const quality = this.qualityEdit.selected['value'];
    const locked = this.lockEdit['checked'];

    const record = this.dataSource.data.find(row => row.id === id);
    const d = this.datePipe.transform(record.date, 'dd/MM/yyyy');
    const changes = Array<{name: string, current: any, new: any}>();

    if (email !== record.email) {
      changes.push({name: 'Email', current: record.email, new: email});
    }

    if (salary !== record.salary) {
      changes.push({name: 'Salary', current: record.salary, new: salary});
    }

    if (date !== d) {
      changes.push({name: 'Date', current: d, new: date});
    }

    if (quality !== record.quality.value) {
      changes.push({name: 'Quality', current: record.quality.value, new: quality});
    }

    if (locked !== record.locked) {
      changes.push({name: 'Locked', current: record.locked, new: locked});
    }
    ...
    if (changes.length) {
      ...
      const dialogRef = this.dialog.open(SavePromptComponent, dialogConfig);
        ...
        if (save) {
          changes.forEach(function(change) {
            switch (change.name) {
              ...
              case 'Date':
                record.date = new Date(date.replace(/(\d{2})\/(\d{2})\/(\d{4})/, '$2/$1/$3'));
                break;

              case 'Quality':
                record.quality = QUALITIES.find(q => q.value === quality);
                break;
  ...
  getColor(value: number) {
    return QUALITIES.find(q => q.value === value).color;
  }
